...
// TODO: Move from perror to fprintf.
...
struct entry populate_entry(char* path) {
  struct stat st;
  struct entry entry;
  struct entry dummy; // TODO: find better place
  dummy.path = NULL;
  ...
  entry.path = path; // TODO: populate real path, for example if its set to "../", it will be "../", not the real path
  entry.st = st;

  // printf("populate_entry %s\n", path);
...
int populate_directory_entry(struct entry* entry) {
  ...
  while((de = readdir(dir)) != NULL) {
    ...
    if(!temp) {
      ...
      perror("An error occured trying to realloc entries.\n");
      return 1;
    }
    ...
    temp_entry = populate_entry(path);
  ...
  printf("populate_directory_entry\n");
  return 0;
}
...
void entry_info(struct entry* entry) {
  ...
  if(!S_ISDIR(entry->st.st_mode)) return;
